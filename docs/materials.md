This section is concerned with the stimulus Selection for German lexicon decision tasks. In lexical decision experiments, participants see a single word or non-word (e.g., a pseudoword, a pronounceable or non-pronounceable letter string) on the screen and respond with a keypress to indicate whether the item is a word or a pseudoword. Items are generally selected to fit specific criteria that depend on the research question. After finding a research question, one of the first steps is to select the items – to identify suitable words and find or generate pseudowords. 
