...
Required resources and issues to consider:
...
- Fixation cross: This aims to prepare the participant for the upcoming trial. This presentation prevents eye movements or attentional blinks that could increase response times.
...
The letter strings should be presented in a different random order for each participant. After every 100 trials, we recommend a short, self-paced break (“Press the space bar when you’re ready to continue”). 
...
In addition to the experimental lexical decision task, participants often fill out a survey with their demographics. Unless it is relevant to the research question, we strongly recommend minimizing the number of additional questions. Every new bit of information reduces the anonymity, and identification based on the data should not be possible by law (i.e., GDPR rules). For example, if location, gender, and highest education are collected, identification in a rural region could already be possible. In addition, fewer questions reduce the testing time for participation. Such a questionnaire should be embedded at the beginning or the end of the experiment. 
...
- Age
- Highest education
- Although gender is often reported and even requested by reviewers, we consider it less relevant, as there is no evidence of gender differences in the cognitive processes underlying reading in German adult readers. This is, however, a research gap in need of evidence. 

## Design options

### Controlled experiment  ![Importance Rating 2](images/rating4.png)
Lexical decision tasks are often used to test a single, specific hypothesis. If one has a small number of available participants and strong theoretical motivation, a controlled experiment is feasible. Here, participants respond to words and non-words which are specifically selected to vary on a given characteristic, while not co-varying on other characteristics that may also affect reading processes (see relevant the section on How to determine the effect of a specific variable based on lexical decision data in [“Materials”](https://trust-network-guidelines.readthedocs.io/en/latest/materials/)). The number of trials of a lexical decision task relies on the trade-off between available time and data quality. On the one hand, as the length of the experiment increases, the likelihood of participant drop-out increases. On the other hand, a short study with only a couple of items is less likely to provide high data quality, as outlying trials, participants, or items may skew the results. Regarding the desired timing, it is reasonable to assume that a typical reading adult can finish a task that takes 30-60 minutes. Such a task length may be suitable for 500-1,000 decisions (e.g., 250 words and 250 non-words), including pre-stimulus and post-response delays and pauses between blocks. 
